...
describe('UsuarioService', () => {
  ...
  describe('Create', () => {
    it('Criar um novo usuario', async () => {
      ...
      const usuarioMock = Object.assign(createusuarioDto, { id: 1 });
      const spyRepositorySave = jest
        .spyOn(repository, 'save')
        .mockResolvedValue(Promise.resolve(usuarioMock));

      const response = await service.create(createusuarioDto);
      expect(response).toEqual(usuarioMock);
      expect(spyRepositorySave).toHaveBeenCalled();
    });

    it('Lançar uma exceção ao repetir um email ja cadastrado, quando criar um novo usuario', async () => {
      const mockUsuario = {
        id: 1,
        nome: 'Teste',
        email: 'teste@test.com',
  ...
  describe('update', () => {
    ...
    it('lançar uma exceção ao enviar ids diferentes quando alterar um usuário', async () => {
      ...
      try {
        ...
        expect(error.message).toBe(EMensagem.IDsDivergente);
      }
    });
